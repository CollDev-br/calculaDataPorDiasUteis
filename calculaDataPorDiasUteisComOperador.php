<?php
function diasUteis($prazo, $data, $operador)
{
    $data_hoje = $data; // Dia atual
    ...
    while ($a < $prazo) {
        $data_passe = date("Y-m-d", strtotime($data_passe . $operador . ' 1 days')); // 
        if (date('w', strtotime($data_passe)) > 0 && date('w', strtotime($data_passe)) < 6) {
            $data_calc = date("Y-m-d", strtotime($data_passe . $operador . ' 1 days'));
            ++$a;
        }
    }
